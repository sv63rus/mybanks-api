...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	...
	case banktranslation.Table:
		...
		return query.Only(ctx)
	case currencyrate.Table:
		query := c.CurrencyRate.Query().
			Where(currencyrate.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, currencyrateImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
	case offer.Table:
		query := c.Offer.Query().
			Where(offer.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, offerImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
...
func (c *Client) noders(ctx context.Context, table string, ids []int) ([]Noder, error) {
	...
	case currencyrate.Table:
		query := c.CurrencyRate.Query().
			Where(currencyrate.IDIn(ids...))
		query, err := query.CollectFields(ctx, currencyrateImplementors...)
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
	...
	case offer.Table:
		query := c.Offer.Query().
			Where(offer.IDIn(ids...))
		query, err := query.CollectFields(ctx, offerImplementors...)
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
