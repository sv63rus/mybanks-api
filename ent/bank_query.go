...
// QueryCurrencyRates chains the current query on the "currency_rates" edge.
func (bq *BankQuery) QueryCurrencyRates() *CurrencyRateQuery {
...
func (bq *BankQuery) QueryOffers() *OfferQuery {
...
// WithCurrencyRates tells the query-builder to eager-load the nodes that are connected to
// the "currency_rates" edge. The optional arguments are used to configure the query builder of the edge.
func (bq *BankQuery) WithCurrencyRates(opts ...func(*CurrencyRateQuery)) *BankQuery {
	query := (&CurrencyRateClient{config: bq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	bq.withCurrencyRates = query
	return bq
}
...
func (bq *BankQuery) WithOffers(opts ...func(*OfferQuery)) *BankQuery {
	query := (&OfferClient{config: bq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (bq *BankQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Bank, error) {
	...
	if query := bq.withCurrencyRates; query != nil {
		if err := bq.loadCurrencyRates(ctx, query, nodes,
			func(n *Bank) { n.Edges.CurrencyRates = []*CurrencyRate{} },
			func(n *Bank, e *CurrencyRate) { n.Edges.CurrencyRates = append(n.Edges.CurrencyRates, e) }); err != nil {
			return nil, err
		}
	}
	if query := bq.withOffers; query != nil {
...
func (bq *BankQuery) loadCurrencyRates(ctx context.Context, query *CurrencyRateQuery, nodes []*Bank, init func(*Bank), assign func(*Bank, *CurrencyRate)) error {
	...
	query.withFKs = true
	query.Where(predicate.CurrencyRate(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(bank.CurrencyRatesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.bank_currency_rates
		if fk == nil {
			return fmt.Errorf(`foreign-key "bank_currency_rates" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "bank_currency_rates" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (bq *BankQuery) loadOffers(ctx context.Context, query *OfferQuery, nodes []*Bank, init func(*Bank), assign func(*Bank, *Offer)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Bank)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Offer(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(bank.OffersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.bank_offers
		if fk == nil {
			return fmt.Errorf(`foreign-key "bank_offers" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
